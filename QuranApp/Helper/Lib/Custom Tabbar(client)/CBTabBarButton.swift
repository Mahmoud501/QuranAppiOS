...
public class CBTabBarButton: UIControl {
    ...
    var BadgeValue:String? {
        didSet {
            if BadgeValue == nil {
                ...
                tabBadge.text = BadgeValue
                tabBadge.removeFromSuperview()
            }else {
                item?.badgeValue = BadgeValue
                tabBadge.text = BadgeValue
    ...
    private func configureSubviews() {
        ...
        tabBadge.frame = CGRect(x: 15, y: 0, width: 20, height: 20)
        tabBadge.layer.cornerRadius = 10
        tabBadge.clipsToBounds = true
        tabBadge.clipsToBounds = true
        tabBadge.textColor = .white
        tabBadge.textAlignment = .center
        tabBadge.font = UIFont(name: "System", size: 5)
        tabBadge.adjustsFontSizeToFitWidth = true
        tabBadge.minimumScaleFactor = 0.5
        tabBadge.text = currentBadge
        tabBadge.backgroundColor = item?.badgeColor
